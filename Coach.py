...
class Coach():
    ...
    def learn(self):
        ...
        for i in range(self.args.start_iteration, self.args.start_iteration + self.args.numIters):
            # bookkeeping
            ...
            self.nnet.train(trainExamples, self.args.checkpoint)
            nmcts = MCTS(self.game, self.nnet, self.args)

            log.info('PITTING AGAINST PREVIOUS VERSION')
            arena = Arena(lambda x, p: np.argmax(pmcts.getActionProb(x, temp=0)),
                          lambda x, p: np.argmax(nmcts.getActionProb(x, temp=0)), self.game)
            ...
            if pwins + nwins == 0 or float(nwins) / (pwins + nwins) < self.args.updateThreshold:
                ...
                self.nnet.save_checkpoint(folder=self.args.checkpoint, filename='{}best.pth.tar'.format(self.args.save_prefix))
